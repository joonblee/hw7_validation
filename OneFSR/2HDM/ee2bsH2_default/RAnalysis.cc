...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      book(_h_dR_rj, "h_dR_rj", 50, 1., 6.);
      book(_h_dR_ij, "h_dR_ij", 100, 0., 5.);

      book(_h_m_ij,  "h_m_ij",  70,100.,800.);

      book(_h_pT,  "h_pT",  100,0.,300.);
      book(_h_qT,  "h_qT",  100,300.,1300.);
    ...
    void analyze(const Event& event) {
      ...
      for(const Particle& p : fs.particles()) {
        ...
        else if(abs(p.pid())==35) {
          out = p;
          NH++;
        }
      }
      if( Nleg!=2 || NH!=1 ) vetoEvent;
      if( abs(recoil.eta()) > 5. ) vetoEvent;
      if( abs(recoil.pt()) < 20. ) vetoEvent;
      if( abs(branch.eta()) > 5. ) vetoEvent;
      if( abs(branch.pt()) < 20. ) vetoEvent;
