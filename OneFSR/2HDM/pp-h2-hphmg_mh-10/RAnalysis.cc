...
namespace Rivet {
  ...
  public:
    ...
    void init() {
      ...
      book(_h_dR_ri, "h_dR_ri", 50, 0., 10.);
      book(_h_dR_rj, "h_dR_rj", 50, 0., 10.);
      book(_h_dR_ij, "h_dR_ij", 100, 0., 5.);

      book(_h_m_ij,  "h_m_ij",  100,0.,100.);

      book(_h_pT,  "h_pT",  50,0.,50.0);
      book(_h_qT,  "h_qT",  120,30.,150.0);
      book(_h_z,   "h_z",   50,0.,1.);
      //book(_n_h, "n_h", 1,0.,1.);
    ...
    void analyze(const Event& event) {
      ...
      Particle recoil, branch, out;
      int NH=0;
      bool found_r(false);
      // find particles
      for(const Particle& p : fs.particles()) {
        if((abs(p.pid())<10||p.pid()==21)&&!found_r) {
          recoil = p;
          found_r = true;
        }
        else if(p.pid()==37) {
